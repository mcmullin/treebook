class UserFriendship < ActiveRecord::Base
  attr_accessible :user, :friend, :user_id, :friend_id, :state
	...
	def self.request(user1, user2)
		transaction do # both are created, or neither are created
			friendship1 = create!(user: user1, friend: user2, state: 'pending') # == 'UserFriendship.create(...)' because we're in the class
			friendship2 = create!(user: user2, friend: user1, state: 'requested') # notice that we're manually setting the state
			
			friendship1.send_request_email
			friendship1 # return statement
		end
	end
